...
CREATE VIEW [Purchase Report]
As

SELECT MIN(Purchase.InvoiceNo) As 'Code', Product.Name As 'Product Name', Category.Name As 'Category Name',SUM( (MRP*Quantity)) As 'Total MRP',
SUM( (UnitPrice*Quantity)) As 'Total Price',SUM( Quantity) As ' Total Quantity', SUM((MRP*Quantity)-(UnitPrice*Quantity)) AS 'Profit'
FROM Purchase 
...
As
SELECT Sales.SalesCode As 'Code', Product.Name As 'Name',Category.Name As 'Category',Sales.Date As 'Date', Sales.Quantity As 'Quantity',Sales.MRP As 'MRP'
FROM Sales
...
As
SELECT MIN( Code) As 'Code', Name, SalesView.Category,Min(Date) As Date, SUM( Quantity) AS 'Quantity',SUM (MRP*Quantity) As 'ToTal MRP',
Sum( PerUnitPrice.Unitprice*Quantity) As 'total Unit Price', SUM((MRP*Quantity)-(PerUnitPrice.Unitprice*Quantity))As Profit FROM SalesView
LEFT JOIN PerUnitPrice ON SalesView.Name = PerUnitPrice.Product AND SalesView.Category=PerUnitPrice.Category

GROUP BY Name,SalesView.Category ;
